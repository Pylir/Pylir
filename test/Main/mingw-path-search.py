...
# RUN: env PATH=%S/Inputs/mingw-single-target-sysroot/bin pylir %s -### --target=x86_64-w64-windows-gnu 2>&1 | FileCheck %s

# CHECK: -L{{.*}}mingw-single-target-sysroot{{[/\\]+}}lib{{[/\\]+}}clang{{[/\\]+}}15.0.4{{[/\\]+}}lib{{[/\\]+}}windows{{[[:space:]]}}
