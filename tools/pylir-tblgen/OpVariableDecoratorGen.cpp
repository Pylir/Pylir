...
public:
    ...
    class MethodGenIter : public llvm::mapped_iterator_base<MethodGenIter, llvm::ListInit::const_iterator, MethodGen>
    {
    public:
        explicit MethodGenIter(llvm::ListInit::const_iterator u) : mapped_iterator_base(u) {}
...
{
    // TODO: def is sadly private in mlir::tblgen::InterfaceMethod, and it doesn't have a `getDef` either. Remove
    //       this once the situation has changed.
    const llvm::Record* m_def;

public:
    explicit ExtendableInterfaceMethod(const llvm::Record* def) : InterfaceMethod(def), m_def(def) {}
...
mlir::FailureOr<ExtendableInterfaceMethod> findExtendableInterfaceMethod(const mlir::tblgen::Interface& interface,
                                                                         const OpVariableGen::MethodGen& methodGen)
{
    const auto* res = llvm::find_if(interface.getMethods(), [&](const mlir::tblgen::InterfaceMethod& method)
                                    { return method.getName() == methodGen.getMethodName(); });
    if (res == interface.getMethods().end())
    {
        llvm::PrintError(methodGen.getDef(), "No method named '" + methodGen.getMethodName() + "' found in interface '"
                                                 + interface.getName() + "'");
        return mlir::failure();
    }
    std::size_t index = res - interface.getMethods().begin();
    const llvm::Record* record = interface.getDef().getValueAsListOfDefs("methods")[index];
    if (!record->isSubClassOf("ExtendableInterfaceMethod"))
    {
        llvm::PrintError(methodGen.getDef(),
                         "'OpVariableGen' can only extend interface method of type 'ExtendableInterfaceMethod'");
        return mlir::failure();
    }
    ...
    collectExtendableInterfaceMethods(const mlir::tblgen::Operator& op)
{
    ...
    // Prime the map by first collecting all 'ExtendableInterfaceMethod' applicable. without doing so it'd lead to no
    // method body being generated in the case of no operands being annotated.
    for (const auto& trait : op.getTraits())
    {
        const auto* interfaceTrait = llvm::dyn_cast<mlir::tblgen::InterfaceTrait>(&trait);
        // Have to special case 'mlir::tblgen::SideEffectTrait' here. It is odd and is a 'mlir::tblgen::InterfaceTrait'
        // but it doesn't actually have a interface. A call to 'getInterface()' below would cause an assertion.
        if (!interfaceTrait || mlir::isa<mlir::tblgen::SideEffectTrait>(interfaceTrait)
    ...
    for (int i = 0; i < op.getNumArgs(); i++)
    {
        for (auto dec : op.getArgDecorators(i))
        {
            ...
            auto* namedCons = op.getArg(i).dyn_cast<mlir::tblgen::NamedTypeConstraint*>();
            if (!namedCons)
            {
                llvm::PrintError(&dec.getDef(), "'OpVariableGen' is currently only supported on operands");
                continue;
            }
            ...
            {
                mlir::FailureOr<ExtendableInterfaceMethod> method = findExtendableInterfaceMethod(interface, methodGen);
                ...
                if (res == result.end())
                {
                    // If during priming, a reason was found not to generate the method body, it won't be present in the
                    // map so just skip it.
                    continue;
                }
...
void emitInterfaceMethodImpl(mlir::raw_indented_ostream& os, const mlir::tblgen::Operator& op,
                             const ExtendableInterfaceMethod& interfaceMethod, llvm::ArrayRef<std::string> bodies)
{
    ...
    auto method = mlir::tblgen::Method(interfaceMethod.getReturnType(), interfaceMethod.getName(),
...
mlir::GenRegistration genIntrinsics("gen-op-variable-decorators", "Generate Op variable decorators", emit);
} // namespace
