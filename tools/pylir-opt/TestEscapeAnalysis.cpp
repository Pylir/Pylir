...
namespace
{
class TestEscapeAnalysisPass : public pylir::test::impl::TestEscapeAnalysisPassBase<TestEscapeAnalysisPass>
{
protected:
    void runOnOperation() override
    {
        for (auto iter : getOperation().getOps<mlir::FunctionOpInterface>())
        {
            ...
            // Print the function first, allowing us to capture SSA names in FileCheck.
            iter->print(llvm::errs());
            llvm::errs() << "\nEscapes: ";
            bool first = true;
            mlir::AsmState state(iter);
            iter.getFunctionBody().walk(
                [&](mlir::Operation* op)
                {
