...
///
mlir::LogicalResult
performActions(llvm::raw_ostream& os, bool verifyPasses,
  ...
  mlir::OwningOpRef<mlir::Operation*> op =
      mlir::parseSourceFileForTool(sourceMgr, config, true);
  ...
  mlir::PassManager pm(context, op.get()->getName().getStringRef(),
...
mlir::LogicalResult processBuffer(
    llvm::raw_ostream& os, std::unique_ptr<llvm::MemoryBuffer> ownedBuffer,
  ...
  if (!verifyDiagnostics) {
    ...
    return performActions(os, verifyPasses, sourceMgr, &context,
  ...
  (void)performActions(os, verifyPasses, sourceMgr, &context,
                       passManagerSetupFn, emitBytecode);
...
mlir::LogicalResult mlirOptMain(
  ...
  auto chunkFn = [&](std::unique_ptr<llvm::MemoryBuffer> chunkBuffer,
    ...
    return processBuffer(os, std::move(chunkBuffer), verifyDiagnostics,
                         verifyPasses, allowUnregisteredDialects, emitBytecode,
                         passManagerSetupFn, registry, threadPool);
  };
  return mlir::splitAndProcessBuffer(std::move(buffer), chunkFn, outputStream,
                                     splitInputFile,
...
int main(int argc, char** argv) {
  ...
  // Reimplementation of MlirOptMain overload that sets up all the command line
  // options. The purpose of doing so is to implicitly run
  // `pylir-finalize-ref-attrs` pass over the whole module before any other
  ...
  // Form a new command line which injects the pylir-finalize-ref-attrs pass as
  // first pass in the pipeline. This is done by detecting use of -p and
  // -pass-pipline and inserting it as very first pass there, or by simply
  ...
  for (auto iter = args.begin(); iter != args.end();) {
    ...
    } else if (text.substr(0, std::string_view("-pass-pipeline=").size()) ==
               "-pass-pipeline=") {
  ...
  if (failed(::mlirOptMain(
          ...
          allowUnregisteredDialects, emitBytecode))) {
    return -1;
  }
