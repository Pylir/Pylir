...
// Called by stackmap_source.ll.in to escape the pointers and extend their lifetime beyond the closure call.
extern "C" void pylir_test_stack_escape(pylir::rt::PyObject& o)
{
    std::cerr << ' ' << &o;
}

// Function defined in stackmap_source.ll.in and compiled by pylir to generate the stackmap. The reason this function
// has so many parameters is so that the chance of at least one being put in a callee saved register is very high,
// regardless of architecture. That way we should have full test coverage of roots on the stack, regardless of precise
// location (spilled, callee save register, alloca).
extern "C" void pylir_test_stack_read(void* closure, void (*closureCall)(void*), pylir::rt::PyObject& a,
...
TEST_CASE("Stackmap reader")
{
    std::array<pylir::rt::PyString, 16> array = {
        pylir::rt::PyString("zero"),     pylir::rt::PyString("first"),      pylir::rt::PyString("second"),
        pylir::rt::PyString("third"),    pylir::rt::PyString("fourth"),     pylir::rt::PyString("fifth"),
        pylir::rt::PyString("sixth"),    pylir::rt::PyString("seventh"),    pylir::rt::PyString("eighth"),
