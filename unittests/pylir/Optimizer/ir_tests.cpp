...
TEST_CASE("IR DictArgsIterator", "[IR]")
{
    ...
    auto vector = std::vector<pylir::Py::DictArg>{
        pylir::Py::DictEntry{block->getArgument(0), block->getArgument(1), block->getArgument(2)},
        pylir::Py::MappingExpansion{block->getArgument(3)},
        pylir::Py::DictEntry{block->getArgument(4), block->getArgument(5), block->getArgument(6)},
        pylir::Py::MappingExpansion{block->getArgument(7)},
    };
    auto op = builder.create<pylir::Py::MakeDictOp>(vector);

    auto range = op.getDictArgs();
    CHECK_THAT(std::vector(range.begin(), range.end()), Catch::Matchers::Equals(vector));
    auto reverse = llvm::reverse(range);
    std::reverse(vector.begin(), vector.end());
    CHECK_THAT(std::vector(reverse.begin(), reverse.end()), Catch::Matchers::Equals(vector));
}
