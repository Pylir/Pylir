...
def main():
    ...
    parser.add_argument(
        '--msvc', dest='msvc', required=False, action='store_true',
        default=False)
    parser.add_argument(
    ...
    ar_exe = args.ar_exe
    if not ar_exe:
        ar_exe = distutils.spawn.find_executable('ar' if not args.msvc else 'lib')
    if not ar_exe:
    ...
    if args.msvc:
        execute_command_verbose([ar_exe, f'/OUT:{args.output}', *archives],
    ...
    if args.use_libtool:
        ...
        execute_command_verbose(
            [libtool_exe, '-static', '-o', args.output] + files,
            cwd=temp_directory_root, verbose=args.verbose)
    else:
