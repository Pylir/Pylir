...
#define DEBUG_TYPE "pylir-place-statepoints"

using namespace llvm;

// Print the liveset found at the insert location
static cl::opt<bool> PrintLiveSet("pylir-spp-print-liveset", cl::Hidden, cl::init(false));
static cl::opt<bool> PrintLiveSetSize("pylir-spp-print-liveset-size", cl::Hidden, cl::init(false));

// Print out the base pointers for debugging
static cl::opt<bool> PrintBasePointers("pylir-spp-print-base-pointers", cl::Hidden, cl::init(false));

// Cost threshold measuring when it is profitable to rematerialize value instead
// of relocating it
static cl::opt<unsigned> RematerializationThreshold("pylir-spp-rematerialization-threshold", cl::Hidden, cl::init(6));
...
static cl::opt<bool, true> ClobberNonLiveOverride("pylir-clobber-non-live", cl::location(ClobberNonLive), cl::Hidden);

static cl::opt<bool> AllowStatepointWithNoDeoptInfo("pylir-allow-statepoint-with-no-deopt-info", cl::Hidden,
                                                    cl::init(true));
...
static bool isRelocatingGC(const GCStrategy&)
{
    // TODO
    return false;
}

PreservedAnalyses pylir::PlaceStatepointsPass::run(Module& M, ModuleAnalysisManager& AM)
{
    bool Changed = false;
...
{
    Optional<OperandBundleUse> DeoptBundle = Call->getOperandBundle(LLVMContext::OB_deopt);

    if (!DeoptBundle.has_value())
    {
        assert(AllowStatepointWithNoDeoptInfo && "Found non-leaf call without deopt info!");
        return None;
    }
...
static void computeLiveInValues(const GCStrategy& strategy, DominatorTree& DT, Function& F, GCPtrLivenessData& Data);

/// Given results from the dataflow liveness computation, find the set of live
/// Values at a particular instruction.
static void findLiveSetAtInst(const GCStrategy& strategy, Instruction* inst, GCPtrLivenessData& Data,
                              StatepointLiveSetTy& out);

static bool isGCPointerType(const GCStrategy& strategy, Type* T)
{
    return strategy.isGCManagedPointer(T) == true;
}
...
// is not encountered.)
static bool isHandledGCPointerType(const GCStrategy& strategy, Type* T)
{
    // We fully support gc pointers
    if (isGCPointerType(strategy, T))
        return true;
    // We partially support vectors of gc pointers. The code will assert if it
    // can't handle something.
    if (auto VT = dyn_cast<VectorType>(T))
        if (isGCPointerType(strategy, VT->getElementType()))
            return true;
    return false;
}
...
static bool containsGCPtrType(const GCStrategy& strategy, Type* Ty)
{
    if (isGCPointerType(strategy, Ty))
        return true;
    if (VectorType* VT = dyn_cast<VectorType>(Ty))
        return isGCPointerType(strategy, VT->getScalarType());
    if (ArrayType* AT = dyn_cast<ArrayType>(Ty))
        return containsGCPtrType(strategy, AT->getElementType());
    if (StructType* ST = dyn_cast<StructType>(Ty))
        return llvm::any_of(ST->elements(), [&](Type* SubTy) { return containsGCPtrType(strategy, SubTy); });
    return false;
}
...
static bool isUnhandledGCPointerType(const GCStrategy& strategy, Type* Ty)
{
    return containsGCPtrType(strategy, Ty) && !isHandledGCPointerType(strategy, Ty);
}
#endif
...
static void analyzeParsePointLiveness(const GCStrategy& strategy, GCPtrLivenessData& OriginalLivenessData,
                                      CallBase* Call, PartiallyConstructedSafepointRecord& Result)
{
    StatepointLiveSetTy LiveSet;
    findLiveSetAtInst(strategy, Call, OriginalLivenessData, LiveSet);
...
struct BaseDefiningValueResult
{
    ...
    BaseDefiningValueResult(Value* BDV, bool IsKnownBase) : BDV(BDV), IsKnownBase(IsKnownBase)
    {
#ifndef NDEBUG
...
static BaseDefiningValueResult findBaseDefiningValueOfVector(Value* I)
{
    ...
    if (isa<Constant>(I))
        ...
        return BaseDefiningValueResult(ConstantAggregateZero::get(I->getType()), true);
    ...
    assert((isa<SelectInst>(I) || isa<PHINode>(I)) && "unknown vector instruction - no base found for vector element");
    return BaseDefiningValueResult(I, false);
}
...
static BaseDefiningValueResult findBaseDefiningValue(Value* I)
{
    assert(I->getType()->isPtrOrPtrVectorTy() && "Illegal to ask for the base pointer of a non-pointer type");
    ...
    if (isa<Constant>(I))
    {
        ...
        return BaseDefiningValueResult(ConstantPointerNull::get(cast<PointerType>(I->getType())), true);
    }
    ...
    if (IntrinsicInst* II = dyn_cast<IntrinsicInst>(I))
    {
        switch (II->getIntrinsicID())
        {
            default:
                ...
                break;
            case Intrinsic::experimental_gc_statepoint: llvm_unreachable("statepoints don't produce pointers");
            ...
            case Intrinsic::experimental_gc_get_pointer_base: return findBaseDefiningValue(II->getOperand(0));
        }
    }
    // We assume that functions in the source language only return base
    // pointers.  This should probably be generalized via attributes to support
    ...
    bool IsKnownBase = isa<Instruction>(I) && cast<Instruction>(I)->getMetadata("is_base_value");
    ...
    assert((isa<SelectInst>(I) || isa<PHINode>(I)) && "missing instruction case in findBaseDefiningValing");
    return BaseDefiningValueResult(I, IsKnownBase);
}
...
static bool isOriginalBaseResult(Value* V)
{
    ...
    return !isa<PHINode>(V) && !isa<SelectInst>(V) && !isa<ExtractElementInst>(V) && !isa<InsertElementInst>(V)
...
static bool isKnownBaseResult(Value* V)
{
    if (isOriginalBaseResult(V))
        return true;
    if (isa<Instruction>(V) && cast<Instruction>(V)->getMetadata("is_base_value"))
    {
...
{
    return isa<VectorType>(First->getType()) == isa<VectorType>(Second->getType());
}
...
public:
    ...
    {
        return OriginalValue == Other.OriginalValue && BaseValue == Other.BaseValue && Status == Other.Status;
    }
    ...
    void print(raw_ostream& OS) const
    {
        switch (getStatus())
        {
            case Unknown: OS << "U"; break;
            case Base: OS << "B"; break;
            case Conflict: OS << "C"; break;
        }
        OS << " (base " << getBaseValue() << " - " << (getBaseValue() ? getBaseValue()->getName() : "nullptr") << ")"
...
static Value* findBasePointer(Value* I, DefiningValueMapTy& Cache)
{
    ...
    {
        return isa<PHINode>(BDV) || isa<SelectInst>(BDV) || isa<ExtractElementInst>(BDV) || isa<InsertElementInst>(BDV)
    ...
    do
    {
        ...
        for (auto Pair : States)
        {
            ...
            bool CanPrune = true;
            visitBDVOperands(BDV, [&](Value* Op) { CanPrune = CanPrune && canPruneInput(Op); });
            if (CanPrune)
    ...
    while (Progress)
    {
        ...
        for (auto Pair : States)
        {
            ...
            // lattice.
            assert((!isKnownBaseResult(BDV) || !areBothVectorOrScalar(BDV, Pair.second.getBaseValue()))
                   && "why did it get added?");

            BDVState NewState(BDV);
            visitBDVOperands(BDV,
                             [&](Value* Op)
                             {
        ...
        assert(OldSize == States.size() && "fixed point shouldn't be adding any new nodes to state");
    }
    ...
    for (auto Pair : States)
    {
        ...
        // type (scalar versus vector) from a possible known base should be in the
        // lattice.
        assert((!isKnownBaseResult(I) || !areBothVectorOrScalar(I, BaseValue)) && "why did it get added?");
        ...
        if (isa<ExtractElementInst>(I))
        {
            ...
            auto* BaseInst = ExtractElementInst::Create(State.getBaseValue(), EE->getIndexOperand(), "base_ee", EE);
    ...
    for (auto Pair : States)
    {
        ...
        // type (scalar versus vector) from a possible known base should be in the
        // lattice.
        assert((!isKnownBaseResult(I) || !areBothVectorOrScalar(I, State.getBaseValue())) && "why did it get added?");
        assert(!State.isUnknown() && "Optimistic algorithm didn't complete!");
    ...
    for (auto Pair : States)
    {
        ...
        // type (scalar versus vector) from a possible known base should be in the
        // lattice.
        assert((!isKnownBaseResult(BDV) || !areBothVectorOrScalar(BDV, State.getBaseValue()))
               && "why did it get added?");
        assert(!State.isUnknown() && "Optimistic algorithm didn't complete!");
        if (!State.isConflict())
            continue;
        ...
        else if (SelectInst* BaseSI = dyn_cast<SelectInst>(State.getBaseValue()))
        {
        ...
        else if (auto* BaseEE = dyn_cast<ExtractElementInst>(State.getBaseValue()))
        {
    ...
    for (auto Pair : States)
    {
        ...
        // type (scalar versus vector) from a possible known base should be in the
        // lattice.
        assert((!isKnownBaseResult(BDV) || !areBothVectorOrScalar(BDV, Base)) && "why did it get added?");

        LLVM_DEBUG(dbgs() << "Updating base value cache"
                          << " for: " << BDV->getName()
                          << " from: " << (Cache.count(BDV) ? Cache[BDV]->getName().str() : "none")
                          << " to: " << Base->getName() << "\n");
...
static void findBasePointers(const StatepointLiveSetTy& live, PointerToBaseTy& PointerToBase,
                             [[maybe_unused]] DominatorTree* DT, DefiningValueMapTy& DVCache)
{
    for (Value* ptr : live)
    {
        ...
        assert((!isa<Instruction>(base) || !isa<Instruction>(ptr)
                || DT->dominates(cast<Instruction>(base)->getParent(), cast<Instruction>(ptr)->getParent()))
...
/// parse point.
static void findBasePointers(DominatorTree& DT, DefiningValueMapTy& DVCache, CallBase* Call,
                             PartiallyConstructedSafepointRecord& result, PointerToBaseTy& PointerToBase)
{
...
static void recomputeLiveInValues(const GCStrategy& strategy, GCPtrLivenessData& RevisedLivenessData, CallBase* Call,
                                  PartiallyConstructedSafepointRecord& result, PointerToBaseTy& PointerToBase);

static void recomputeLiveInValues(const GCStrategy& strategy, Function& F, DominatorTree& DT,
                                  ...
                                  PointerToBaseTy& PointerToBase)
{
    ...
    computeLiveInValues(strategy, DT, F, RevisedLivenessData);
    for (size_t i = 0; i < records.size(); i++)
    {
        struct PartiallyConstructedSafepointRecord& info = records[i];
        recomputeLiveInValues(strategy, RevisedLivenessData, toUpdate[i], info, PointerToBase);
    }
}
...
static BasicBlock* normalizeForInvokeSafepoint(BasicBlock* BB, BasicBlock* InvokeParent, DominatorTree& DT)
{
    ...
    assert(!isa<PHINode>(Ret->begin()) && "All PHI nodes should have been removed!");
...
static constexpr Attribute::AttrKind FnAttrsToStrip[] = {Attribute::ReadNone,
                                                         Attribute::ReadOnly,
                                                         Attribute::WriteOnly,
...
static void CreateGCRelocates([[maybe_unused]] const GCStrategy& strategy, ArrayRef<Value*> LiveVariables,
                              ArrayRef<Value*> BasePtrs, Instruction* StatepointToken, IRBuilder<>& Builder)
{
    ...
    {
        assert(isHandledGCPointerType(strategy, Ty));
        auto AS = Ty->getScalarType()->getPointerAddressSpace();
        Type* NewTy = Type::getInt8PtrTy(M->getContext(), AS);
        if (auto* VT = dyn_cast<VectorType>(Ty))
            NewTy = FixedVectorType::get(NewTy, cast<FixedVectorType>(VT)->getNumElements());
        return Intrinsic::getDeclaration(M, Intrinsic::experimental_gc_relocate, {NewTy});
    };
    ...
    for (unsigned i = 0; i < LiveVariables.size(); i++)
    {
        ...
        CallInst* Reloc = Builder.CreateCall(GCRelocateDecl, {StatepointToken, BaseIdx, LiveIdx},
...
public:
    ...
    {
        assert(Old != New && Old && New && "Cannot RAUW equal values or to / from null!");
    ...
    void doReplacement()
    {
        ...
        assert((!IsDeoptimize || !New) && "Deoptimize intrinsics are not replaced!");
...
static void makeStatepointExplicitImpl(const GCStrategy& strategy, CallBase* Call, /* to replace */
                                       ...
                                       const PointerToBaseTy& PointerToBase)
{
    ...
    StatepointDirectives SD = parseStatepointDirectivesFromAttrs(Call->getAttributes());
    if (SD.NumPatchBytes)
    ...
    if (Function* F = dyn_cast<Function>(CallTarget.getCallee()))
    {
        ...
        if (IID == Intrinsic::memcpy_element_unordered_atomic || IID == Intrinsic::memmove_element_unordered_atomic)
        {
            ...
            auto GetBaseAndOffset = [&](Value* Derived)
            {
                ...
                Value* Base_int = Builder.CreatePtrToInt(Base, Type::getIntNTy(Context, IntPtrSize));
                Value* Derived_int = Builder.CreatePtrToInt(Derived, Type::getIntNTy(Context, IntPtrSize));
            ...
            auto GetFunctionName = [](Intrinsic::ID IID, ConstantInt* ElementSizeCI)
            {
                ...
                if (IID == Intrinsic::memcpy_element_unordered_atomic)
                {
                    switch (ElementSize)
                    {
                        case 1: return "__llvm_memcpy_element_unordered_atomic_safepoint_1";
                        case 2: return "__llvm_memcpy_element_unordered_atomic_safepoint_2";
                        case 4: return "__llvm_memcpy_element_unordered_atomic_safepoint_4";
                        case 8: return "__llvm_memcpy_element_unordered_atomic_safepoint_8";
                        case 16: return "__llvm_memcpy_element_unordered_atomic_safepoint_16";
                        default: llvm_unreachable("unexpected element size!");
                    }
                }
                assert(IID == Intrinsic::memmove_element_unordered_atomic);
                switch (ElementSize)
                {
                    case 1: return "__llvm_memmove_element_unordered_atomic_safepoint_1";
                    case 2: return "__llvm_memmove_element_unordered_atomic_safepoint_2";
                    case 4: return "__llvm_memmove_element_unordered_atomic_safepoint_4";
                    case 8: return "__llvm_memmove_element_unordered_atomic_safepoint_8";
                    case 16: return "__llvm_memmove_element_unordered_atomic_safepoint_16";
                    default: llvm_unreachable("unexpected element size!");
                }
            };

            CallTarget = F->getParent()->getOrInsertFunction(GetFunctionName(IID, ElementSizeCI), FTy);
        }
    }
    ...
    if (auto* CI = dyn_cast<CallInst>(Call))
    {
        CallInst* SPCall = Builder.CreateGCStatepointCall(
            ...
            isRelocatingGC(strategy) ? Optional<ArrayRef<Value*>>{} : makeArrayRef(BasePtrs),
            isRelocatingGC(strategy) ? LiveVariables : ArrayRef<Value*>{}, "safepoint_token");
        ...
        // gc_result intrinsic.
        SPCall->setAttributes(legalizeCallAttributes(CI->getContext(), CI->getAttributes(), SPCall->getAttributes()));
    ...
    else
    {
        ...
        InvokeInst* SPInvoke = Builder.CreateGCStatepointInvoke(
            StatepointID, NumPatchBytes, CallTarget, II->getNormalDest(), II->getUnwindDest(), CallArgs,
            isRelocatingGC(strategy) ? Optional<ArrayRef<Value*>>{} : makeArrayRef(BasePtrs),
            isRelocatingGC(strategy) ? LiveVariables : ArrayRef<Value*>{}, "statepoint_token");
        ...
        SPInvoke->setAttributes(
            legalizeCallAttributes(II->getContext(), II->getAttributes(), SPInvoke->getAttributes()));

        Token = cast<GCStatepointInst>(SPInvoke);

        Builder.SetCurrentDebugLocation(II->getDebugLoc());

        if (isRelocatingGC(strategy))
        {
            // Generate gc relocates in exceptional path
            BasicBlock* UnwindBlock = II->getUnwindDest();
            assert(!isa<PHINode>(UnwindBlock->begin()) && UnwindBlock->getUniquePredecessor()
            ...
            CreateGCRelocates(strategy, LiveVariables, BasePtrs, ExceptionalToken, Builder);
        }
        // Generate gc relocates and returns for normal block
        BasicBlock* NormalDest = II->getNormalDest();
        assert(!isa<PHINode>(NormalDest->begin()) && NormalDest->getUniquePredecessor()
               && "can't safely insert in this block!");
    ...
    if (!Call->getType()->isVoidTy() && !Call->use_empty())
    {
        ...
        GCResult->setAttributes(AttributeList::get(GCResult->getContext(), AttributeList::ReturnIndex,
    ...
    if (isRelocatingGC(strategy))
    {
        ...
        CreateGCRelocates(strategy, LiveVariables, BasePtrs, Token, Builder);
    }
}
...
static void makeStatepointExplicit(const GCStrategy& strategy, CallBase* Call,
                                   PartiallyConstructedSafepointRecord& Result,
                                   std::vector<DeferredReplacement>& Replacements, const PointerToBaseTy& PointerToBase)
{
    ...
    makeStatepointExplicitImpl(strategy, Call, BaseVec, LiveVec, Result, Replacements, PointerToBase);
}
                                   ...
                                   [[maybe_unused]] DenseSet<Value*>& VisitedLiveValues)
{
    for (User* U : GCRelocs)
    {
        ...
        assert(Relocate->getNextNode() && "Should always have one since it's not a terminator");
        IRBuilder<> Builder(Relocate->getNextNode());
        Value* CastedRelocatedValue = Builder.CreateBitCast(Relocate, cast<AllocaInst>(Alloca)->getAllocatedType(),
                                                            suffixed_name_or(Relocate, ".casted", ""));

        new StoreInst(CastedRelocatedValue, Alloca, cast<Instruction>(CastedRelocatedValue)->getNextNode());
                                          ...
                                          [[maybe_unused]] DenseSet<Value*>& VisitedLiveValues)
{
    for (auto RematerializedValuePair : RematerializedValues)
    {
        ...
        assert(AllocaMap.count(OriginalValue) && "Can not find alloca for rematerialized value");
        Value* Alloca = AllocaMap[OriginalValue];

        new StoreInst(RematerializedValue, Alloca, RematerializedValue->getNextNode());
...
static void relocationViaAlloca(Function& F, DominatorTree& DT, ArrayRef<Value*> Live,
                                ArrayRef<PartiallyConstructedSafepointRecord> Records)
{
    ...
    {
        AllocaInst* Alloca =
            new AllocaInst(LiveValue->getType(), DL.getAllocaAddrSpace(), "", F.getEntryBlock().getFirstNonPHI());
    ...
    for (const auto& Info : Records)
    {
        ...
        {
            insertRelocationStores(Info.UnwindToken->users(), AllocaMap, VisitedLiveValues);
        }

        // Do similar thing with rematerialized values
        insertRematerializationStores(Info.RematerializedValues, AllocaMap, VisitedLiveValues);
    ...
    for (auto Pair : AllocaMap)
    {
        ...
        for (Instruction* Use : Uses)
        {
            ...
            {
                LoadInst* Load = new LoadInst(Alloca->getAllocatedType(), Alloca, "", Use);
        ...
        // inserted before it.
        StoreInst* Store = new StoreInst(Def, Alloca, /*volatile*/ false, DL.getABITypeAlign(Def->getType()));
...
static void insertUseHolderAfter(CallBase* Call, const ArrayRef<Value*> Values, SmallVectorImpl<CallInst*>& Holders)
{
    ...
    FunctionCallee Func =
        M->getOrInsertFunction("__tmp_use", FunctionType::get(Type::getVoidTy(M->getContext()), true));
    if (isa<CallInst>(Call))
    {
        // For call safepoints insert dummy calls right after safepoint
        Holders.push_back(CallInst::Create(Func, Values, "", &*++Call->getIterator()));
        return;
    }
    // For invoke safepooints insert dummy calls both in normal and
    // exceptional destination blocks
    auto* II = cast<InvokeInst>(Call);
    Holders.push_back(CallInst::Create(Func, Values, "", &*II->getNormalDest()->getFirstInsertionPt()));
    Holders.push_back(CallInst::Create(Func, Values, "", &*II->getUnwindDest()->getFirstInsertionPt()));
}

static void findLiveReferences(const GCStrategy& strategy, Function& F, DominatorTree& DT, ArrayRef<CallBase*> toUpdate,
                               MutableArrayRef<struct PartiallyConstructedSafepointRecord> records)
{
    GCPtrLivenessData OriginalLivenessData;
    computeLiveInValues(strategy, DT, F, OriginalLivenessData);
    for (size_t i = 0; i < records.size(); i++)
    {
        struct PartiallyConstructedSafepointRecord& info = records[i];
        analyzeParsePointLiveness(strategy, OriginalLivenessData, toUpdate[i], info);
    }
}
...
static Value* findRematerializableChainToBasePointer(SmallVectorImpl<Instruction*>& ChainToBase, Value* CurrentValue)
{
    if (GetElementPtrInst* GEP = dyn_cast<GetElementPtrInst>(CurrentValue))
    {
        ...
        return findRematerializableChainToBasePointer(ChainToBase, GEP->getPointerOperand());
    }
    ...
    if (CastInst* CI = dyn_cast<CastInst>(CurrentValue))
    {
        ...
        return findRematerializableChainToBasePointer(ChainToBase, CI->getOperand(0));
    }
...
static InstructionCost chainToBasePointerCost(SmallVectorImpl<Instruction*>& Chain, TargetTransformInfo& TTI)
{
    ...
    for (Instruction* Instr : Chain)
    {
        if (CastInst* CI = dyn_cast<CastInst>(Instr))
        {
            ...
            Cost += TTI.getCastInstrCost(CI->getOpcode(), CI->getType(), SrcTy, TTI::getCastContextHint(CI),
...
static bool AreEquivalentPhiNodes(PHINode& OrigRootPhi, PHINode& AlternateRootPhi)
{
    ...
    if (PhiNum != AlternateRootPhi.getNumIncomingValues() || OrigRootPhi.getParent() != AlternateRootPhi.getParent())
        return false;
    ...
    for (unsigned i = 0; i < PhiNum; i++)
        CurrentIncomingValues[OrigRootPhi.getIncomingValue(i)] = OrigRootPhi.getIncomingBlock(i);
    ...
    for (unsigned i = 0; i < PhiNum; i++)
    {
        auto CIVI = CurrentIncomingValues.find(AlternateRootPhi.getIncomingValue(i));
...
static void findRematerializationCandidates(PointerToBaseTy PointerToBase, RematCandTy& RematerizationCandidates,
                                            TargetTransformInfo& TTI)
{
    ...
    for (auto P2B : PointerToBase)
    {
        ...
        SmallVector<Instruction*, 3> ChainToBase;
        Value* RootOfChain = findRematerializableChainToBasePointer(ChainToBase, Derived);
...
static void rematerializeLiveValues(CallBase* Call, PartiallyConstructedSafepointRecord& Info,
                                    PointerToBaseTy& PointerToBase, RematCandTy& RematerizationCandidates)
{
    ...
    for (Value* LiveValue : Info.LiveSet)
    {
        ...
        auto rematerializeChain =
            [&ChainToBase](Instruction* InsertBefore, Value* RootOfChain, Value* AlternateLiveBase)
        {
            ...
            for (Instruction* Instr : ChainToBase)
            {
                ...
                if (LastClonedValue)
                {
                    ...
                    for (auto OpValue : ClonedValue->operand_values())
                    {
                        ...
                        assert(!is_contained(ChainToBase, OpValue) && "incorrect use in rematerialization chain");
        ...
        if (isa<CallInst>(Call))
        {
            ...
            Instruction* RematerializedValue =
                rematerializeChain(InsertBefore, Record.RootOfChain, PointerToBase[LiveValue]);
        ...
        else
        {
            ...
            Instruction* NormalInsertBefore = &*Invoke->getNormalDest()->getFirstInsertionPt();
            Instruction* UnwindInsertBefore = &*Invoke->getUnwindDest()->getFirstInsertionPt();

            Instruction* NormalRematerializedValue =
                rematerializeChain(NormalInsertBefore, Record.RootOfChain, PointerToBase[LiveValue]);
            Instruction* UnwindRematerializedValue =
                rematerializeChain(UnwindInsertBefore, Record.RootOfChain, PointerToBase[LiveValue]);
    ...
    for (auto* Callsite : Intrinsics)
        switch (Callsite->getIntrinsicID())
        {
            case Intrinsic::experimental_gc_get_pointer_base:
            {
                ...
                auto* BaseBC =
                    IRBuilder<>(Callsite).CreateBitCast(Base, Callsite->getType(), suffixed_name_or(Base, ".cast", ""));
                if (BaseBC != Base)
                    DVCache[BaseBC] = Base;
            ...
            case Intrinsic::experimental_gc_get_pointer_offset:
            {
                ...
                Value* BaseInt = Builder.CreatePtrToInt(Base, Type::getIntNTy(Context, IntPtrSize),
                                                        suffixed_name_or(Base, ".int", ""));
                Value* DerivedInt = Builder.CreatePtrToInt(Derived, Type::getIntNTy(Context, IntPtrSize),
...
static bool insertParsePoints(const GCStrategy& strategy, Function& F, DominatorTree& DT, TargetTransformInfo& TTI,
                              SmallVectorImpl<CallBase*>& ToUpdate, DefiningValueMapTy& DVCache)
{
#ifndef NDEBUG
    // Validate the input
    ...
    for (CallBase* Call : ToUpdate)
    {
        ...
        {
            assert(!isUnhandledGCPointerType(strategy, Arg->getType()) && "support for FCA unimplemented");
            if (isHandledGCPointerType(strategy, Arg->getType()))
    ...
    // site.
    findLiveReferences(strategy, F, DT, ToUpdate, Records);
    ...
    // not the key issue.
    recomputeLiveInValues(strategy, F, DT, ToUpdate, Records, PointerToBase);
    ...
    if (isRelocatingGC(strategy))
    {
        ...
        for (size_t i = 0; i < Records.size(); i++)
            rematerializeLiveValues(ToUpdate[i], Records[i], PointerToBase, RematerizationCandidates);
    }
    ...
    for (size_t i = 0; i < Records.size(); i++)
        makeStatepointExplicit(strategy, ToUpdate[i], Records[i], Replacements, PointerToBase);
    ...
    for (size_t i = 0; i < Records.size(); i++)
    {
        ...
        for (Value* V : Info.StatepointToken->gc_args())
        {
            ...
            assert(DT.isReachableFromEntry(LiveInst->getParent()) && "unreachable values should never be live");
    ...
    // Validation check
    for (auto* Ptr : Live)
        assert(isHandledGCPointerType(strategy, Ptr->getType()) && "must be a gc pointer type");
#endif
...
static void stripInvalidMetadataFromInstruction(Instruction& I)
{
    ...
    unsigned ValidMetadataAfterRS4GC[] = {
        LLVMContext::MD_tbaa,    LLVMContext::MD_range, LLVMContext::MD_alias_scope, LLVMContext::MD_nontemporal,
        LLVMContext::MD_nonnull, LLVMContext::MD_align, LLVMContext::MD_type};
...
static bool shouldRewriteStatepointsIn(Function& F)
{
    ...
    if (F.hasGC())
    {
        ...
        return "pylir-gc" == FunctionGCName;
    }
    else
        return false;
}
...
bool pylir::PlaceStatepointsPass::runOnFunction(Function& F, DominatorTree& DT, TargetTransformInfo& TTI,
                                                const TargetLibraryInfo& TLI)
{
    assert(!F.isDeclaration() && !F.empty() && "need function body to rewrite statepoints in");
    ...
    auto NeedsRewrite = [&TLI](Instruction& I)
    {
        if (const auto* Call = dyn_cast<CallBase>(&I))
        {
            ...
            if (!AllowStatepointWithNoDeoptInfo && !Call->getOperandBundle(LLVMContext::OB_deopt))
            {
    ...
    for (Instruction& I : instructions(F))
    {
        ...
        if (NeedsRewrite(I))
        {
            ...
            assert(DT.isReachableFromEntry(I.getParent()) && "no unreachable blocks expected");
    ...
    for (Instruction& I : instructions(F))
    {
        ...
        for (unsigned i = 0; i < I.getNumOperands(); i++)
            ...
            {
                assert(VF == 0 || VF == cast<FixedVectorType>(OpndVTy)->getNumElements());
    ...
    auto gcInfo = llvm::getGCStrategy(F.getGC());
    ...
    if (!ParsePointNeeded.empty())
        MadeChange |= insertParsePoints(*gcInfo, F, DT, TTI, ParsePointNeeded, DVCache);
...
static void computeLiveInValues(const GCStrategy& strategy, BasicBlock::reverse_iterator Begin,
                                BasicBlock::reverse_iterator End, SetVector<Value*>& LiveTmp)
{
    for (auto& I : make_range(Begin, End))
    {
        ...
        {
            assert(!isUnhandledGCPointerType(strategy, V->getType()) && "support for FCA unimplemented");
            if (isHandledGCPointerType(strategy, V->getType()) && !isa<Constant>(V))
            {
...
static void computeLiveOutSeed(const GCStrategy& strategy, BasicBlock* BB, SetVector<Value*>& LiveTmp)
{
...
static SetVector<Value*> computeKillSet(const GCStrategy& strategy, BasicBlock* BB)
{
    SetVector<Value*> KillSet;
    for (Instruction& I : *BB)
        if (isHandledGCPointerType(strategy, I.getType()))
            KillSet.insert(&I);
    return KillSet;
}
...
static void checkBasicSSA(DominatorTree& DT, SetVector<Value*>& Live, Instruction* TI, bool TermOkay = false)
{
    for (Value* V : Live)
    {
        if (auto* I = dyn_cast<Instruction>(V))
        {
            ...
            if (TermOkay && TI == I)
                continue;
            assert(DT.dominates(I, TI) && "basic SSA liveness expectation violated by liveness analysis");
        }
    }
}
...
static void computeLiveInValues(const GCStrategy& strategy, [[maybe_unused]] DominatorTree& DT, Function& F,
                                GCPtrLivenessData& Data)
{
    ...
    for (BasicBlock& BB : F)
    {
        Data.KillSet[&BB] = computeKillSet(strategy, &BB);
        Data.LiveSet[&BB].clear();
        computeLiveInValues(strategy, BB.rbegin(), BB.rend(), Data.LiveSet[&BB]);
        ...
        computeLiveOutSeed(strategy, &BB, Data.LiveOut[&BB]);
                              ...
                              StatepointLiveSetTy& Out)
{
    ...
    computeLiveInValues(strategy, BB->rbegin(), ++Inst->getIterator().getReverse(), LiveOut);
    LiveOut.remove(Inst);
    Out.insert(LiveOut.begin(), LiveOut.end());
}

static void recomputeLiveInValues(const GCStrategy& strategy, GCPtrLivenessData& RevisedLivenessData, CallBase* Call,
                                  PartiallyConstructedSafepointRecord& Info, PointerToBaseTy& PointerToBase)
{
    StatepointLiveSetTy Updated;
    findLiveSetAtInst(strategy, Call, RevisedLivenessData, Updated);
