...
class BestFitTree
{
    ...
    public:
        ...
        BlockHeader(std::size_t size, BlockHeader* previousBlock) : m_previousBlock(previousBlock), size(size) {}

        [[nodiscard]] BlockHeader* getPreviousBlock() const noexcept
        {
            return reinterpret_cast<BlockHeader*>(reinterpret_cast<std::uintptr_t>(m_previousBlock)
                                                  & ~std::uintptr_t{0b11});
        }
        ...
        void setPreviousBlock(BlockHeader* block) noexcept
        {
            m_previousBlock = reinterpret_cast<BlockHeader*>(
                (reinterpret_cast<std::uintptr_t>(block)) | (reinterpret_cast<std::uintptr_t>(m_previousBlock) & 0b11));
        }

        [[nodiscard]] BlockHeader* getNextBlock() const noexcept
        {
            return reinterpret_cast<BlockHeader*>(
                const_cast<std::byte*>(reinterpret_cast<const std::byte*>(this) + sizeof(BlockHeader) + size));
        }
        ...
        [[nodiscard]] Balance getBalance() const noexcept
        {
            ...
            return static_cast<Balance>(reinterpret_cast<std::uintptr_t>(m_previousBlock) & 0b11);
        }

        void setBalance(Balance balance) noexcept
        {
            m_previousBlock = reinterpret_cast<BlockHeader*>(
                (reinterpret_cast<std::uintptr_t>(m_previousBlock) & ~std::uintptr_t{0b11}) | balance);
        }

        Node& emplaceNode() noexcept
        {
            setBalance(Equal);
            return *new (reinterpret_cast<std::byte*>(this) + sizeof(BlockHeader)) Node{};
        }

        [[nodiscard]] Node& getNode() noexcept
        {
            PYLIR_ASSERT(!isAllocated());
            return *reinterpret_cast<Node*>(reinterpret_cast<std::byte*>(this) + sizeof(BlockHeader));
        }
...
public:
    explicit BestFitTree(std::size_t lowerBlockSizeLimit) : m_lowerBlockSizeLimit(lowerBlockSizeLimit)
    {
