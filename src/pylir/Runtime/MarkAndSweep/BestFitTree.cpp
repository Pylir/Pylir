...
void pylir::rt::BestFitTree::swapNode(BlockHeader* lhs, BlockHeader* rhs)
{
    ...
    auto fixUpPointees = [this](BlockHeader* header, BlockHeader* previous)
    {
        ...
        for (auto member : {&Node::left, &Node::right, &Node::parent, &Node::multiNext, &Node::multiPrevious})
        {
...
template <pylir::rt::BestFitTree::BlockHeader* pylir::rt::BestFitTree::Node::*direction>
auto pylir::rt::BestFitTree::rotate(BlockHeader* root) -> BlockHeader*
{
    constexpr auto otherDirection = direction == &Node::left ? &Node::right : &Node::left;
...
void pylir::rt::BestFitTree::leftRotate(BlockHeader* parent, BlockHeader* current)
{
...
void pylir::rt::BestFitTree::rightRotate(BlockHeader* parent, BlockHeader* current)
{
...
void pylir::rt::BestFitTree::rightLeftRotate(BlockHeader* parent, BlockHeader* current)
{
...
void pylir::rt::BestFitTree::leftRightRotate(BlockHeader* parent, BlockHeader* current)
{
...
void pylir::rt::BestFitTree::remove(BlockHeader* current)
{
    ...
    auto& node = current->getNode();
    for (auto member : {&Node::left, &Node::right, static_cast<BlockHeader * Node::*>(nullptr)})
    {
...
pylir::rt::PyObject* pylir::rt::BestFitTree::alloc(std::size_t size)
{
    ...
    if (!result)
    {
        ...
        result = new (memory.get()) BlockHeader(memory.size() - 2 * sizeof(BlockHeader), nullptr);
        // Sentinel for end
        new (memory.get() + memory.size() - sizeof(BlockHeader)) BlockHeader(0, result);
    }
...
void pylir::rt::BestFitTree::free(PyObject* object)
{
    ...
    auto* blockHeader = reinterpret_cast<BlockHeader*>(reinterpret_cast<std::byte*>(object) - sizeof(BlockHeader));
    ...
    auto unlinkBlock = [this](BlockHeader* block)
    {
        ...
        if (!prev)
        {
            if (next)
            {
                // Instead of a remove and then a reinsert, swap with the next node to save a rebalance operation. They
                // have the same key anyways
                swapNode(next, block);
            }
            else
            {
        ...
        {
            // If we are not part of the AVL tree we can simply remove ourselves from the doubly linked list
