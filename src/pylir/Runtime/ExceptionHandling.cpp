...
Result findLandingPad(_Unwind_Action actions, bool nativeException, _Unwind_Exception* exception,
        ...
        while (true)
        {
            ...
            if (typeIndex > 0)
            {
                ...
                if (!nativeException && typeObject == &pylir::rt::Builtins::BaseException)
                {
                    ...
                    //       of cleanup as finally code may actually stop the exception
            ...
            {
                // Don't support filters or anything of the sort at the moment as I don't have a need yet
            }
            else
            {
                // cleanup clause
                                    ...
                                    _Unwind_Exception* exceptionInfo, _Unwind_Context* context)
{
    ...
    {
        auto* pyException = pylir::rt::PyBaseException::fromUnwindHeader(exceptionInfo);
        _Unwind_SetGR(context, __builtin_eh_return_data_regno(0), reinterpret_cast<uintptr_t>(exceptionInfo));
        _Unwind_SetGR(context, __builtin_eh_return_data_regno(1), static_cast<uintptr_t>(pyException->getTypeIndex()));
        _Unwind_SetIP(context, pyException->getLandingPad());
        return _URC_INSTALL_CONTEXT;
    }
    auto result = findLandingPad(actions, nativeException, exceptionInfo, context);
    if (result.code == _URC_CONTINUE_UNWIND || result.code == _URC_FATAL_PHASE1_ERROR)
    {
        return result.code;
    }
    if (actions & _UA_SEARCH_PHASE)
    ...
    // TODO: Trying to jump to a catch handler with a foreign exception. Figure out how that is supposed
    //       to work
    _Unwind_SetGR(context, __builtin_eh_return_data_regno(0), reinterpret_cast<uintptr_t>(exceptionInfo));
    _Unwind_SetGR(context, __builtin_eh_return_data_regno(1), static_cast<uintptr_t>(result.typeIndex));
