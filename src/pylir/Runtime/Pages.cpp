...
void __attribute__((constructor(200))) pageSizeInit()
{
    ...
    // This is technically not the page size, but it is granularity VirtualAlloc works with.
...
#ifdef _WIN32
    return {reinterpret_cast<std::byte*>(VirtualAlloc(nullptr, bytes, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE)),
            bytes};
#else
    return {
        reinterpret_cast<std::byte*>(mmap(nullptr, bytes, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, 0, 0)),
        bytes};
#endif
}
