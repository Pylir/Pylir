...
constexpr auto O4_MAY_ENABLE_LTO_COMPILER_MIGHT_OUTPUT_LLVM_IR =
    FMT_STRING("'-O4' may enable LTO. Compiler might output LLVM IR instead of an {}");
...
constexpr auto FAILED_TO_CREATE_TEMPORARY_FILE_N = FMT_STRING("failed to create temporary file '{}'");

constexpr auto FAILED_TO_DISCARD_TEMPORARY_FILE_N = FMT_STRING("failed to discard temporary file '{}'");
...
constexpr auto ADDRESS_AND_THREAD_SANITIZERS_ARE_INCOMPATIBLE_WITH_EACH_OTHER =
    FMT_STRING("'address' and 'thread' sanitizers are incompatible with each other");
