...
mlir::LogicalResult pylir::DiagnosticsVerifier::checkExpected(const Diag::Diagnostic::Message& message,
                                                              decltype(m_fileLineToExpectedMessages)::iterator res)
{
    ...
    auto expectedIter = llvm::find_if(res->second,
...
pylir::DiagnosticsVerifier::DiagnosticsVerifier(Diag::DiagnosticsManager& manager)
{
    manager.setDiagnosticCallback(
        [&](Diag::Diagnostic&& diagnostic)
        {
            for (auto& iter : diagnostic.messages)
            {
                auto res = m_fileLineToExpectedMessages.find(
                    {iter.document, iter.document ? iter.document->getLineNumber(iter.location) : 0});
                if (mlir::failed(checkExpected(iter, res)))
                {
                    m_errorsOccurred = true;
                    std::vector<Diag::Diagnostic::Message> messages;
                    messages.push_back({Diag::Severity::Error, nullptr, 0, "Did not expect diagnostic:", {}});
    ...
    for (const auto& iter : ctre::multiline_range<PATTERN>(document.getText()))
    {
        ...
        for (const auto& iter2 : ctre::multiline_range<TEXT_WITHIN_PATTERN>(iter.get<TEXT>()))
        {
            ...
            while (!strToProcess.empty())
            {
                ...
                regexOS << llvm::Regex::escape(Text::toUTF8String(strToProcess.substr(0, regexIt)));
                ...
                {
                    m_errorsOccurred = true;
                    auto openBracketPos = static_cast<std::size_t>(strToProcess.data() - document.begin()) - 2;
                    llvm::errs() << Diag::DiagnosticsBuilder(document, Diag::Severity::Error, openBracketPos,
                                                             "found start of regex with no end '}}}}'")
                                        .addHighlight(openBracketPos, openBracketPos + 1)
                                        .getDiagnostic();
                    continue;
                }
                std::string regexStr = Text::toUTF8String(strToProcess.substr(0, regexEndIt));
                ...
                if (!llvm::Regex(regexStr).isValid(regexError))
                {
                    ...
                    llvm::errs() << Diag::DiagnosticsBuilder(document, Diag::Severity::Error, regexStart,
                                                             "invalid regex '{}'", regexStr)
                                        .addHighlight(regexStart, regexEnd - 1, std::move(regexError))
                                        .getDiagnostic();
                    continue;
                }
            ...
            m_fileLineToExpectedMessages[{&document, line}].push_back(
                {start, end, severity, std::make_unique<llvm::Regex>(std::move(regexRes)), ""});
        }
    }
}
...
mlir::LogicalResult pylir::DiagnosticsVerifier::verify()
{
    for (auto& iter : m_fileLineToExpectedMessages)
    {
        ...
        for (auto& iter2 : iter.second)
        {
            ...
            llvm::errs() << Diag::DiagnosticsBuilder(*iter.first.first, Diag::Severity::Error, iter2.start,
                                                     "Did not encounter {} at line {}:", strRep, iter.first.second)
