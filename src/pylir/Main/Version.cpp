...
std::optional<pylir::Version> pylir::Version::parse(llvm::StringRef version)
{
    ...
    if (splits.size() == 2)
    {
        if (auto end = splits[1].find_first_not_of("0123456789"); end != llvm::StringRef::npos)
        {
            result.suffix = splits[1].substr(end);
            splits[1] = splits[1].substr(0, end);
        }
    }
    if (splits[1].getAsInteger(10, result.minorVersion) || result.minorVersion < 0)
    {
        return {};
    }
    if (splits.size() < 3)
    ...
    if (auto end = splits[2].find_first_not_of("0123456789"); end != llvm::StringRef::npos)
    {
