...
/// It is essentially a list of strings that are passed to the linker. Its value comes from the addition of 'add'
/// methods for common options and concepts used across all supported linkers, without having to strictly follow
/// any linkers concrete command line syntax.
/// Follows the builder pattern for being able to chain modification methods.
...
public:
    /// Constructs the builder using a particular linker style. This will be used for the final linker invocation as
    /// indicator which linker should be called and is also important for the higher level 'add' methods, to determine
    /// what command line syntax to use.
    explicit LinkerInvocationBuilder(LinkerStyle linkerStyle) : m_linkerStyle(linkerStyle) {}
    ...
    /// Overload of the above to prevent accidentally using a string literal as condition.
    LinkerInvocationBuilder& addArg(llvm::Twine argument, const char*) = delete;

    /// Adds a list of raw arguments to the linker command that are directly forwarded to the linker.
    ...
    /// Adds the emulation option to the linker, telling it about the target architecture.
    /// Only allowed for ELF and MinGW style!
    ...
    LinkerInvocationBuilder& addLibrarySearchDirs(llvm::ArrayRef<std::string> directories)
    {
    ...
    /// Add a directory, composed of 'args', interleaved with the native file separator, to the library search path of
    /// the linker.
    template <class First, class... Args, std::enable_if_t<(sizeof...(Args) > 0)>* = nullptr>
    LinkerInvocationBuilder& addLibrarySearchDir(First&& first, Args&&... args)
    {
        addLibrarySearchDir((first + ... + (llvm::sys::path::get_separator() + args)));
        return *this;
    }
