...
mlir::Value
buildTrySpecialMethodCall(pylir::PyBuilder& builder, TypeSlots method,
                          mlir::Value args, mlir::Value kws,
                          mlir::Block* notFoundPath,
                          mlir::Block* callIntrException = nullptr) {
  auto element = builder.createTupleGetItem(
      args, builder.create<mlir::arith::ConstantIndexOp>(0));
  ...
  builder.create<mlir::cf::CondBranchOp>(builder.getCurrentLoc(), failure,
                                         notFoundPath, exec);
  ...
  builder.create<mlir::cf::CondBranchOp>(
      builder.getCurrentLoc(), failure, mergeBlock,
  ...
  auto result = builder.createPylirCallIntrinsic(getMethod.getResult(), tuple,
                                                 ...
                                                 callIntrException);
  builder.create<mlir::cf::BranchOp>(builder.getCurrentLoc(), mergeBlock,
                                     result);
...
mlir::Value buildSpecialMethodCall(pylir::PyBuilder& builder, TypeSlots method,
                                   mlir::Value args, mlir::Value kws,
                                   mlir::Block* callIntrException = nullptr) {
...
mlir::Value binOp(pylir::PyBuilder& builder, TypeSlots method,
                  TypeSlots revMethod, mlir::Value lhs, mlir::Value rhs) {
  ...
  builder.create<mlir::cf::CondBranchOp>(
      sameType, normalMethodBlock, mlir::ValueRange{trueC}, differentTypeBlock,
      mlir::ValueRange{});
  ...
  builder.create<mlir::cf::CondBranchOp>(
      subclass, isSubclassBlock, normalMethodBlock, mlir::ValueRange{falseC});
  ...
  builder.create<mlir::cf::CondBranchOp>(failure, normalMethodBlock,
                                         mlir::ValueRange{falseC},
                                         hasReversedBlock, mlir::ValueRange{});
  ...
  builder.create<mlir::cf::CondBranchOp>(failure, callReversedBlock,
                                         lhsHasReversedBlock);

  implementBlock(builder, lhsHasReversedBlock);
  auto sameImplementation =
      builder.createIs(lookup.getResult(), lhsLookup.getResult());
  builder.create<mlir::cf::CondBranchOp>(sameImplementation, normalMethodBlock,
                                         mlir::ValueRange{falseC},
  ...
  builder.create<mlir::cf::CondBranchOp>(isNotImplemented, normalMethodBlock,
                                         mlir::ValueRange{trueC}, endBlock,
  ...
  builder.create<mlir::cf::CondBranchOp>(isNotImplemented, maybeTryReverse,
                                         endBlock, mlir::ValueRange{result});

  implementBlock(builder, maybeTryReverse);
  auto* actuallyTryReverse = new mlir::Block;
  builder.create<mlir::cf::CondBranchOp>(normalMethodBlock->getArgument(0),
  ...
  isNotImplemented =
      builder.createIs(reverseResult, builder.createNotImplementedRef());
  builder.create<mlir::cf::CondBranchOp>(isNotImplemented, typeErrorBlock,
                                         endBlock,
                                         mlir::ValueRange{reverseResult});
  ...
  if (method != TypeSlots::Eq && method != TypeSlots::Ne) {
    auto typeError =
        pylir::buildException(builder, TypeError.name, {}, nullptr);
...
void buildRevBinOpCompilerBuiltin(pylir::PyBuilder& builder,
...
void buildBinOpCompilerBuiltin(pylir::PyBuilder& builder,
                               llvm::StringRef functionName, TypeSlots method) {
  auto func = builder.create<pylir::Py::FuncOp>(
      functionName, builder.getFunctionType(
                        {builder.getDynamicType(), builder.getDynamicType()},
                        builder.getDynamicType()));
...
void buildUnaryOpCompilerBuiltin(pylir::PyBuilder& builder,
                                 llvm::StringRef functionName,
                                 TypeSlots method) {
  auto func = builder.create<pylir::Py::FuncOp>(
...
void buildCallOpCompilerBuiltin(pylir::PyBuilder& builder,
                                llvm::StringRef functionName) {
  auto func = builder.create<pylir::Py::FuncOp>(
      functionName, builder.getFunctionType({builder.getDynamicType(),
                                             builder.getDynamicType(),
  ...
  builder.create<mlir::cf::CondBranchOp>(isFunction, isFunctionBlock,
                                         notFunctionBlock);
...
void buildTernaryOpCompilerBuiltin(pylir::PyBuilder& builder,
                                   llvm::StringRef functionName,
                                   TypeSlots method) {
  auto func = builder.create<pylir::Py::FuncOp>(
      functionName, builder.getFunctionType({builder.getDynamicType(),
...
void buildIOpCompilerBuiltins(pylir::PyBuilder& builder,
                              llvm::StringRef functionName, TypeSlots method,
                              mlir::Value (pylir::PyBuilder::*normalOp)(
                                  mlir::Value, mlir::Value, mlir::Block*)) {
  auto func = builder.create<pylir::Py::FuncOp>(
      functionName, builder.getFunctionType(
                        {builder.getDynamicType(), builder.getDynamicType()},
                        builder.getDynamicType()));
  ...
  auto res = builder.createPylirCallIntrinsic(
      lookup.getResult(), builder.createMakeTuple({lhs, rhs}, nullptr),
      builder.createMakeDict());
  auto isNotImplemented =
  ...
  builder.create<mlir::cf::CondBranchOp>(isNotImplemented, fallback,
                                         returnBlock, res);
  ...
  builder.create<mlir::cf::CondBranchOp>(isNotImplemented, throwBlock,
                                         returnBlock, res);
  ...
  builder.create<pylir::Py::ReturnOp>(builder.getCurrentLoc(),
                                      returnBlock->getArgument(0));
}

void buildGetAttributeOpCompilerBuiltin(pylir::PyBuilder& builder,
                                        llvm::StringRef functionName,
                                        TypeSlots method) {
  auto func = builder.create<pylir::Py::FuncOp>(
      functionName, builder.getFunctionType(
  ...
  builder.create<mlir::cf::CondBranchOp>(isAttributeError, getattrBlock,
                                         reraiseBlock);
...
#define COMPILER_BUILTIN_IOP(name, slotName, normalOp)                         \
  buildIOpCompilerBuiltins(                                                    \
      m_builder, COMPILER_BUILTIN_SLOT_TO_API_NAME(slotName), TypeSlots::name, \
      &::pylir::PyBuilder::createPylir##normalOp##Intrinsic);

#include <pylir/Interfaces/CompilerBuiltins.def>

  buildGetAttributeOpCompilerBuiltin(m_builder, "pylir__getattribute__",
