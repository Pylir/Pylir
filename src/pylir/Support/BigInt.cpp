...
{
    static_assert(std::numeric_limits<double>::radix == 2 && std::numeric_limits<double>::is_iec559);
    ...
    // easily be calculated by counting the trailing zeros and subtracting the size of the mantissa.
    // In the case the fraction part is 0, we insert the implicit 1, to get a shift amount of 0.
    std::uint64_t fractionPart = bitCopy.mantissa | (1uLL << DoubleRepr::mantissaSize);
    unsigned shiftRequired = DoubleRepr::mantissaSize - countTrailingZeros(fractionPart);
