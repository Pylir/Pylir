...
class BufferComponent
{
    ...
    static_assert(typename std::allocator_traits<Allocator<T>>::is_always_equal{}, "Allocator must be stateless");
...
public:
    ...
    {
        if constexpr (std::is_same_v<typename std::iterator_traits<InputIter>::iterator_category,
