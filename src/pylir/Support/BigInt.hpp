...
class BigInt
{
    ...
    friend BigInt powmod(const BigInt& base, const BigInt& expo, const BigInt& mod);
...
public:
    ...
    [[nodiscard]] std::string toString(std::uint8_t radix = 10) const
    {
        ...
        // mp_to_radix includes the null terminator in size. We don't do that around here
