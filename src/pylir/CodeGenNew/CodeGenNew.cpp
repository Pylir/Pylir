...
public:
    CodeGenNew(mlir::MLIRContext* context, Diag::DiagnosticsDocManager& manager, CodeGenOptions&& options)
        : m_options(std::move(options)),
          m_builder(mlir::UnknownLoc::get(context), context),
          m_module(m_builder.create<mlir::ModuleOp>()),
          m_docManager(&manager)
    {
        context->loadDialect<Py::PylirPyDialect, HIR::PylirHIRDialect, mlir::cf::ControlFlowDialect>();
    }
    ...
    {
        return variant.match([=](const auto& sub) -> decltype(auto) { return visit(sub); });
    }

    template <class... Args>
    decltype(auto) visit(const std::variant<Args...>& variant)
    {
        return pylir::match(variant, [=](const auto& sub) -> decltype(auto) { return visit(sub); });
    }
    ...
    mlir::ModuleOp visit(const Syntax::FileInput& fileInput)
    {
        ...
        auto init = m_builder.create<HIR::InitOp>(m_options.qualifier.empty() ? "__main__" : m_options.qualifier);
...
private:
    ...
    void visitImpl(const Syntax::FuncDef& funcDef)
    {
        ...
        for (const Syntax::Parameter& iter : funcDef.parameterList)
        {
            switch (iter.kind)
            {
                ...
                {
                    specs.emplace_back(m_builder.getStringAttr(iter.name.getValue()), visit(iter.maybeDefault));
                    break;
                }
                case Syntax::Parameter::PosOnly: specs.emplace_back(); break;
                case Syntax::Parameter::KeywordOnly:
                    specs.emplace_back(m_builder.getStringAttr(iter.name.getValue()), visit(iter.maybeDefault), true);
                    break;
                case Syntax::Parameter::PosRest: specs.emplace_back(HIR::FunctionParameterSpec::PosRest{}); break;
        ...
        auto function = m_builder.create<HIR::FuncOp>(funcDef.funcName.getValue(), specs);
    ...
    void visitImpl([[maybe_unused]] const Syntax::AssignmentStmt& assignmentStmt)
    {
        // TODO:
        PYLIR_UNREACHABLE;
    }
    ...
    void visitImpl([[maybe_unused]] const Syntax::GlobalOrNonLocalStmt& globalOrNonLocalStmt)
    {
        // TODO:
        PYLIR_UNREACHABLE;
    }
    ...
    mlir::Value visitImpl([[maybe_unused]] const Syntax::Conditional& conditional)
    {
        // TODO:
        PYLIR_UNREACHABLE;
    }
    ...
    mlir::Value visitImpl([[maybe_unused]] const Syntax::Subscription& subscription)
    { // TODO:
        PYLIR_UNREACHABLE;
    }
    ...
    mlir::Value visitImpl([[maybe_unused]] const Syntax::TupleConstruct& tupleConstruct)
    {
        // TODO:
        PYLIR_UNREACHABLE;
    }
    ...
    mlir::Value visitImpl([[maybe_unused]] const Syntax::AttributeRef& attributeRef)
    {
        // TODO:
        PYLIR_UNREACHABLE;
    }
    ...
    mlir::Value visitImpl([[maybe_unused]] const Syntax::ListDisplay& listDisplay)
    {
        // TODO:
        PYLIR_UNREACHABLE;
    }
    ...
    mlir::Value visitImpl([[maybe_unused]] const Syntax::DictDisplay& dictDisplay)
    {
        // TODO:
        PYLIR_UNREACHABLE;
    }
...
mlir::OwningOpRef<mlir::ModuleOp> pylir::codegenNew(mlir::MLIRContext* context, const Syntax::FileInput& input,
