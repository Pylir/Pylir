...
mlir::OwningOpRef<mlir::ModuleOp> pylir::linkModules(llvm::MutableArrayRef<mlir::OwningOpRef<mlir::ModuleOp>> modules)
{
    ...
    for (auto& iter : rest)
    {
        ...
        for (auto& newOps :
             llvm::make_early_inc_range(llvm::make_range(std::next(back.getIterator()), operations.end())))
        {
