...
class ConditionalsImplicationsPass final
    : public pylir::impl::ConditionalsImplicationsPassBase<ConditionalsImplicationsPass>
{
    std::optional<mlir::DialectInterfaceCollection<pylir::DialectImplicationPatternsInterface>> m_interfaces;
...
llvm::DenseSet<std::pair<mlir::Block*, mlir::Block*>> collectBackedges(mlir::Region& region)
{
    ...
    for (auto iter = llvm::df_ext_begin(&region, state), end = llvm::df_ext_end(&region, state); iter != end; ++iter)
    {
        for (mlir::Block* succ : iter->getSuccessors())
        {
            // A back edge within a DFS walk is simply an edge to a block whose children have not yet all been visited
            // by the DFS walk. LLVMs abstraction calls this a "not completed" block.
...
/// More specifically, it modifies the DFS walk order to turn into a topologically sorted order.
/// Since the CFG may contain loops however, it simply ignores these loops by pretending back-edges do not exist,
/// therefore once again forming a perfect DAG.
/// This leads to no correctness issues in this pass, just potentially missed optimizations.
...
public:
    ...
    explicit TopoSortState(mlir::Region& region) : m_backEdges(collectBackedges(region)) {}
...
void removeSameBlockSuccessors(llvm::SmallVectorImpl<std::pair<mlir::Block*, mlir::Attribute>>& implications)
{
    if (implications.empty())
    {
        return;
    }
    llvm::sort(implications, [](auto& lhs, auto& rhs) { return lhs.first < rhs.first; });

    // Delete all entries for a block, if the block appears more than once. Since the array is rather sorted this is a
    // rather simple state machine where we switch to "delete previous" as soon as the previous element has the same
    // block as the current.
    bool mustDeleteAll = false;
    for (auto* iter = std::next(implications.begin()); iter != implications.end(); iter++)
    {
...
pylir::ImplicationPatternBase* replaceUseWithConstantPattern(mlir::Value condition, mlir::Attribute value,
                                           pylir::PatternAllocator& allocator)
{
    struct UseRewriter final : pylir::ImplicationPatternBase
    {
        ...
        UseRewriter(mlir::Value condition, mlir::Attribute attribute) : condition(condition), attribute(attribute) {}

        mlir::LogicalResult matchAndRewrite(mlir::Operation* operation, mlir::OpBuilder& builder) const override
        {
            bool changed = false;
            ...
            for (mlir::OpOperand& operand : operation->getOpOperands())
            {
                ...
                {
                    changed = true;
                    for (mlir::Dialect* dialect : operation->getContext()->getLoadedDialects())
                    {
                        mlir::Operation* constantOp = dialect->materializeConstant(
                            builder, attribute, operand.get().getType(), operand.get().getLoc());
                        if (!constantOp)
                        {
                            continue;
                        }
                    ...
                    PYLIR_ASSERT(replacement && "No loaded dialect found to materialize the given constant");
                }
...
// Using SetVector since we iterate over all patterns and want deterministic iteration order.
...
FactList getActiveFactsFromPredecessors(mlir::Block* block, FactMap& facts, const TopoSortState& topoSortState)
{
    ...
    {
        // Ignore back-edges. In regular loops, the loop header always dominates the whole loop body, therefore any
        // facts propagated from the header into the loop remain true.
        // Even in an irregular loop, the facts in all cycle entry blocks would meet and not cause any miscompilations.
        ...
        {
            // Fast path. If a predecessor does not have any facts then the intersection will always be empty.
            return {};
        }
...
bool ConditionalsImplicationsPass::simplifyRegion(mlir::Region& region)
{
    ...
    for (mlir::Block* block : llvm::depth_first_ext(&region, topoSortState))
    {
        ...
        FactList factList = getActiveFactsFromPredecessors(block, facts, topoSortState);
        if (!factList.empty())
        {
        ...
        auto interface = mlir::dyn_cast<pylir::ConditionalBranchInterface>(block->getTerminator());
        if (!interface)
        {
            continue;
        }
        ...
        llvm::SmallVector<std::pair<mlir::Block*, mlir::Attribute>> implications = interface.getBranchImplications();
        ...
        for (auto [branch, startValue] : implications)
        {
            ...
            llvm::SmallVector<std::pair<mlir::Value, mlir::Attribute>> workList = {{startCondition, startValue}};

            while (!workList.empty())
            {
                auto [condition, value] = workList.pop_back_val();

                for (const pylir::DialectImplicationPatternsInterface& dialectInterface : *m_interfaces)
                {
                    dialectInterface.getImplicationPatterns(
                        allocator, condition, value,
                        /*patternAddCallback=*/
                        [&](pylir::ImplicationPatternBase* pattern) { patternList.insert(pattern); },
                ...
                patternList.insert(replaceUseWithConstantPattern(condition, value, allocator));
            }
        }
    }
