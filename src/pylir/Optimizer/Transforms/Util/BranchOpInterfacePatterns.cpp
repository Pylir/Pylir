...
struct PassthroughArgRemove : mlir::OpInterfaceRewritePattern<mlir::BranchOpInterface>
{
    ...
    mlir::LogicalResult matchAndRewrite(mlir::BranchOpInterface op, mlir::PatternRewriter& rewriter) const override
    {
        ...
        for (const auto& iter : llvm::enumerate(op->getSuccessors()))
        {
            ...
            // If the block has more than one operation (aka more than just the terminator), the optimization is not
            // valid as the produced values may not be calculated nor their side effects executed.
            ...
            if (llvm::any_of(successorOperands.getForwardedOperands(),
                             [op](mlir::Value value) { return value.getDefiningOp() == op; })
            ...
            rewriter.updateRootInPlace(
                ...
                [&]
                {
                    ...
                    op.getSuccessorOperands(iter.index()).append(successorOperands.getForwardedOperands());
                });
        }
...
// bb1:
// br bb1()
struct TrivialBlockArgRemove : mlir::OpInterfaceRewritePattern<mlir::BranchOpInterface>
{
    using mlir::OpInterfaceRewritePattern<mlir::BranchOpInterface>::OpInterfaceRewritePattern;
    ...
    mlir::LogicalResult matchAndRewrite(mlir::BranchOpInterface op, mlir::PatternRewriter& rewriter) const override
    {
        ...
        for (const auto& [index, succ] : llvm::enumerate(op->getSuccessors()))
        {
            ...
            mlir::SuccessorOperands successorOperands = op.getSuccessorOperands(index);
            for (std::size_t i = successorOperands.getProducedOperandCount(); i < successorOperands.size(); i++)
            {
                auto succBlockArg = *op.getSuccessorBlockArgument(successorOperands.getOperandIndex(i));
                mlir::Value same;
                for (auto pred = succBlockArg.getOwner()->pred_begin(); pred != succBlockArg.getOwner()->pred_end();
                     pred++)
                {
            ...
            auto remainingBlockArgs = llvm::make_filter_range(succ->getArguments(), [&](mlir::BlockArgument arg)
                                                              { return !skipList.contains(arg); });

            newSucc->addArguments(
                llvm::to_vector(llvm::map_range(remainingBlockArgs, std::mem_fn(&mlir::BlockArgument::getType))),
                llvm::to_vector(llvm::map_range(remainingBlockArgs, std::mem_fn(&mlir::BlockArgument::getLoc))));
            ...
            for (auto pred = succ->pred_begin(); pred != succ->pred_end(); pred++)
            {
                ...
                auto operands = terminator.getSuccessorOperands(pred.getSuccessorIndex());
