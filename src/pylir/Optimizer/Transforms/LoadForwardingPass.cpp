...
void LoadForwardingPass::runOnOperation()
{
    ...
    memorySSA.getMemoryRegion().walk(
        [&](pylir::MemSSA::MemoryUseOp use)
        {
            ...
            // For every read, check that each is defined by the memDef. In other words, that the memDef
            // definitely writes to each of them (potentially via aliasing effects).
            // TODO: It doesn't have to be necessary that every read is defined, just that at least one is. This would
            //       require communicating to the MemoryFoldInterface which read definitely writes from given def.
            //       Implement this once an op requires it.
            // TODO: If there is ever an op with multiple writes it'll likely be necessary to communicate to
            //       MemoryFoldInterface, which write affected a read value.
            for (const auto& read : use.getReads())
            {
                if (!read)
                {
                    // If the read is just a general read, not to a specific location, there is absolutely no way
                    // to figure out whether it definitely reads from the def.
                    return;
                }
                if (!llvm::any_of(defOp.getWrites(),
