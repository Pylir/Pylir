...
protected:
    void runOnOperation() override
    {
        ...
        getOperation()->walk(
            [&](pylir::Mem::GCAllocObjectOp allocObjectOp)
            {
                llvm::APInt trailingObjects;
                if (!mlir::matchPattern(allocObjectOp.getTrailingItems(), mlir::m_ConstantInt(&trailingObjects)))
                {
                    return;
                }
                ...
                auto layoutType = pylir::Mem::getLayoutType(allocObjectOp.getTypeObject(), &cache);
                if (!layoutType)
                {
                    return;
                }
                ...
                for (mlir::Operation* iter : allocObjectOp->getUsers())
                {
                    ...
                    PYLIR_ASSERT(iter->getNumResults() == 1 && "Expected only a single result");
                ...
                auto stackAlloc = builder.create<pylir::Mem::StackAllocObjectOp>(
                    allocObjectOp->getLoc(), allocObjectOp.getTypeObject(), *layoutType, trailingObjects);
