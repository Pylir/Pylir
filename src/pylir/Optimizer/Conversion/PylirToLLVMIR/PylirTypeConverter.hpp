...
/// MLIR Dialect conversion type converter, inheriting LLVM type conversions and adding type conversions from Pylir
/// types to LLVM. Also includes various other functions related to translating Pylir types to LLVM.
...
public:
    ...
    /// Returns the LLVM representation for 'object', optionally with the given slot count.
    mlir::LLVM::LLVMStructType getPyObjectType(std::optional<unsigned> slotSize = {});

    /// Returns the LLVM representation for 'function', optionally with the given slot count.
    mlir::LLVM::LLVMStructType getPyFunctionType(std::optional<unsigned> slotSize = {});

    /// Returns the LLVM representation for a 'tuple', optionally of the given length.
    mlir::LLVM::LLVMStructType getPyTupleType(std::optional<unsigned> length = {});

    /// Returns the LLVM representation for 'list', optionally with the given slot count.
    mlir::LLVM::LLVMStructType getPyListType(std::optional<unsigned> slotSize = {});

    /// Returns the LLVM representation for a buffer component, used within 'dict' and 'str'.
    mlir::LLVM::LLVMStructType getBufferComponent();

    /// Returns the LLVM representation for 'dict', optionally with the given slot count.
    mlir::LLVM::LLVMStructType getPyDictType(std::optional<unsigned> slotSize = {});

    /// Returns the LLVM representation for 'str', optionally with the given slot count.
    ...
    /// Returns the LLVM representation for 'int', optionally with the given slot count.
    mlir::LLVM::LLVMStructType getPyIntType(std::optional<unsigned> slotSize = {});

    /// Returns the LLVM representation for 'float', optionally with the given slot count.
    mlir::LLVM::LLVMStructType getPyFloatType(std::optional<unsigned> slotSize = {});

    /// Returns the LLVM representation for the Itanium ABI's unwind exception object for the compilation target.
    mlir::Type getUnwindHeaderType()
    {
        return m_cabi->getUnwindExceptionHeader(&getContext());
    }

    /// Returns the LLVM representation for 'BaseException', optionally with the given slot count.
    mlir::LLVM::LLVMStructType getPyBaseExceptionType(std::optional<unsigned> slotSize = {});

    /// Returns the LLVM representation for 'type', optionally with the given slots.
    mlir::LLVM::LLVMStructType getPyTypeType(std::optional<unsigned> slotSize = {});

    /// Maps from 'pyMem's layout type enum to the corresponding LLVM type representation, optionally with the
    /// given slot count.
    ...
    /// This is a simple wrapper around 'pylir::Mem::getLayoutType'. See its description for more details.
    std::optional<pylir::Mem::LayoutType> getLayoutType(mlir::Attribute attr);

    /// Returns the LLVM type representation for 'objectAttr'. The type has the required size and storage to store
    /// 'objectAttr', including its slots.
    ...
    /// Returns the name of the root section. The root section is a section for all translated 'py.globalHandle'
    /// global variables. These are collected into an array during code generation and used by the runtime to find
    ...
    /// Returns the name of the collection section. The collection is the section for all translated 'py.globalValue'
    /// variables that are not const and may therefore contain references to GC allocated objects. These are collected
    /// into an array during code generation and used by the runtime to find still referenced objects.
    mlir::StringAttr getCollectionSection() const
    {
        return m_collectionSection;
    }

    /// Returns the name of the constant sections. The constant section is the section for all translated const
    /// 'py.globalValue' variables and 'py.constant' attributes. While it cannot contain references to any GC allocated
    /// objects, objects within this section must still be known to the runtime, to avoid marking by the GC and to
