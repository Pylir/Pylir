...
private:
    ...
    mlir::AliasResult isCorrespondingPartition(mlir::Value value, mlir::AliasAnalysis& aliasAnalysis) const;
...
public:
    ...
    /// Inserts the value into its corresponding AliasSet. Does nothing if it has previously been inserted.
    void insert(mlir::Value value);

    /// Returns the AliasSet that the value is contained in. If the value has not previously been inserted the behaviour
    /// is undefined.
