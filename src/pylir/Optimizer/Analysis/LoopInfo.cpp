...
void pylir::LoopInfo::discoverAndMapSubLoops(pylir::Loop* loop, llvm::ArrayRef<mlir::Block*> latches,
                                             mlir::DominanceInfo& domInfo)
{
    ...
    while (!workList.empty())
    {
        ...
        if (!subLoop)
        {
            ...
            {
                // We have reached the header and hence no more predecessors are part of the loop.
                // Since the header per definition dominates the whole loop, this is inevitable.
                continue;
            }
        ...
        // We discovered a loop within our loop. Change the surrounding/most top level loop in the hierarchy
        // to be this newly discovered one.
...
pylir::LoopInfo::LoopInfo(mlir::Operation* operation, mlir::AnalysisManager& analysisManager)
{
    auto& region = operation->getRegion(0);
    // DominanceInfo does not allow fetching a dominator tree on a single block region. The entry block of a region
    // can also not be the successor of any other block, that means including itself, hence there are no loops in such
    // a region.
    ...
    for (const auto& iter : llvm::post_order(domInfo.getRootNode(&region)))
    {
        ...
        {
            if (domInfo.dominates(maybeHeader, latch) && domInfo.isReachableFromEntry(latch))
            {
    ...
    for (auto* iter : llvm::post_order(&region.front()))
    {
        ...
        if (loop->getHeader() == iter)
        {
            ...
            // For convenience, especially when printing, inverse the order to get from a post order to a reverse
            // post order
            ...
            // The header does not need to be added to the block set/list of the loop as that is already done in the
            // constructor of loop
